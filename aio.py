...
def cmd_ssh():
    ...
    if wda == 'setup': ip=sp.run(['hostname','-I'],capture_output=True,text=True).stdout.split()[0]; u=os.environ.get('USER','user'); ok=sp.run(['pgrep','sshd'],capture_output=True).returncode==0; print(f"This device: {DEVICE_ID} ({u}@{ip})\nSSH server: {'✓ running' if ok else 'x not running - sudo apt install openssh-server && sudo systemctl start ssh'}\n\nTo connect here from another device, run there:\n  aio ssh {DEVICE_ID} {u}@{ip}"); return
