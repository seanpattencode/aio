...
def _write_tmux_conf():
    ...
    sh_min = '#[range=user|new]New#[norange] #[range=user|close]Close#[norange] #[range=user|edit]Edit#[norange] #[range=user|kill]Kill#[norange] #[range=user|detach]Quit#[norange]'
...
set-hook -g alert-silence 'refresh-client -S'
...
def create_tmux_session(session_name, work_dir, cmd, env=None, capture_output=True):
    ...
    if cmd and any(a in cmd for a in ['codex', 'claude', 'gemini']):
        ...
        # Idle monitor: uses pane TTY mtime (immune to clicks, only tracks actual pane output). 🔴 idle, 🟢 active
        sp.Popen(['bash', '-c', f's="";while tmux has-session -t {shlex.quote(session_name)} 2>/dev/null;do tty=$(tmux display-message -t {shlex.quote(session_name)}:0.1 -p "#{{pane_tty}}" 2>/dev/null);[ -n "$tty" ]&&[ -e "$tty" ]&&{{ t=$(stat -c %Y "$tty" 2>/dev/null);printf -v n "%(%s)T" -1;d=$((n-t));if [ $d -gt {REDDOT_IDLE} ];then [ "$s" != r ]&&tmux set-option -t {shlex.quote(session_name)} set-titles-string "🔴 #S:#W";s=r;else [ "$s" != g ]&&tmux set-option -t {shlex.quote(session_name)} set-titles-string "🟢 #S:#W";s=g;fi; }};sleep .2;done'], stdout=sp.DEVNULL, stderr=sp.DEVNULL)
    return result
...
if [ -n "$winner" ] && [ -d "{candidates_dir}/$winner" ]; then
    ...
    # Red dot diagnostic: show idle status for all agent sessions, optionally start monitors
    print("🔴 Red Dot Status\n")
    now = int(time.time())
    for session in sp.run(['tmux', 'list-sessions', '-F', '#{session_name}'], capture_output=True, text=True).stdout.strip().split('\n'):
        if not session: continue
        act = sp.run(['tmux', 'display-message', '-t', session, '-p', '#{window_activity}'], capture_output=True, text=True).stdout.strip()
