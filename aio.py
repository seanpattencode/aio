...
def cmd_ssh():
    ...
    if len(sys.argv) > 3 and sys.argv[3] not in ('cmd', '--cmd'): pw=sys.argv[4] if len(sys.argv)>4 else input("Password? ").strip() or None; pw and _pw(wda,pw); (c:=db()).execute("INSERT OR REPLACE INTO ssh(name,host) VALUES(?,?)",(wda,sys.argv[3])); c.commit(); print(f"✓ Added {wda}={sys.argv[3]}{' [pw]' if pw else ''}"); return
