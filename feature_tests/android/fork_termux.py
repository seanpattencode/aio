...
def rebrand():
    """Change applicationId + package name so fork coexists with stock Termux."""
    ...
    # point TERMUX_PACKAGE_NAME to new id so file paths resolve correctly
    c = re.sub(r'(TERMUX_PACKAGE_NAME\s*=\s*)"[^"]+"', f'\\1"{PKG}"', c)
    # rename app in launcher
    c = re.sub(r'(TERMUX_APP_NAME\s*=\s*)"[^"]+"', r'\1"aio"', c)
    open(bg, 'w').write(c)
    # update hardcoded package name in Java constants (runtime package context lookup)
    tc = f"{APP}/termux-shared/src/main/java/com/termux/shared/termux/TermuxConstants.java"
    j = open(tc).read()
    j = re.sub(r'(TERMUX_PACKAGE_NAME\s*=\s*)"com\.termux"', f'\\1"{PKG}"', j)
    ...
    s = re.sub(r'(ENTITY TERMUX_PACKAGE_NAME )"[^"]+"', f'\\1"{PKG}"', s)
    open(sx, 'w').write(s)
