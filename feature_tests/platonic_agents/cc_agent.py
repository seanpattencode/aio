#!/usr/bin/env python3
"""Minimal agent using claude code CLI (-p flag) for text output."""
import subprocess as S,sys,os
...
while u:=input("\n> ").strip():
    ...
    for _ in range(5):
        p=SYS+"\n\n"+"\n".join(m[-20:])
        t=S.run(["claude","-p"],input=p,capture_output=1,text=1,env=E).stdout.strip()
