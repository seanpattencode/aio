...
def monte_carlo(n=1000, verbose=False):
    ...
    counts={d:(len(list((ROOT/d).glob('*.txt'))),len(list((ROOT/d/'.archive').glob('*.txt')))if(ROOT/d/'.archive').exists()else 0) for d in DEVICES}

    return {
        'actions': n,
        'errors': len(errors),
        'conflicts': len(conflicts),
        'conflict_details': conflicts[:5] if conflicts else [],  # first 5
        'reseeds': reseeds,
        'counts': counts,
