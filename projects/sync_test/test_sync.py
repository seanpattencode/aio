import subprocess as sp, time, shlex
...
def sync(device): ts=time.strftime('%Y%m%dT%H%M%S')+f'.{time.time_ns()%1000000000:09d}'; [p.rename(p.with_name(f'{p.stem}_{ts}{p.suffix}')) for p in (ROOT/device).glob('*.txt') if '_20' not in p.stem]; pull(device); sp.run(f'cd {q(ROOT/device)} && git add -A && git commit -qm "sync" && git push origin main', shell=True)
...
def sync_edit(device): ts=time.strftime('%Y%m%dT%H%M%S')+f'.{time.time_ns()%1000000000:09d}'; arc=(ROOT/device/'.archive'); arc.mkdir(exist_ok=True); [((arc/p.name).write_text(sp.run(f'git -C {q(ROOT/device)} show HEAD:{p.name}',shell=True,capture_output=True,text=True).stdout), p.rename(p.with_name(f'{p.stem.rsplit("_",1)[0]}_{ts}{p.suffix}'))) for p in (ROOT/device).glob('*.txt') if sp.run(f'git -C {q(ROOT/device)} diff --quiet {p.name}',shell=True).returncode]; pull(device); sp.run(f'cd {q(ROOT/device)} && git add -A && git commit -qm "sync" && git push origin main',shell=True)
...
def monte_carlo(n=1000):
    ...
    for i in range(n):
        ...
        try:
            ...
            elif op=='delete' and files: random.choice(files).unlink(); sp.run(f'cd {q(ROOT/d)} && git add -A && git commit -qm "del" && git push origin main',shell=True)
            elif op=='archive' and files: (ROOT/d/'.archive').mkdir(exist_ok=True); f=random.choice(files); f.rename(ROOT/d/'.archive'/f.name); sp.run(f'cd {q(ROOT/d)} && git add -A && git commit -qm "arc" && git push origin main',shell=True)
