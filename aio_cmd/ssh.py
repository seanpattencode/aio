...
def run():
    ...
    if wda == 'self': sys.platform=='darwin' and sp.run(['nc','-z','localhost','22'],capture_output=True).returncode and (input("Remote Login off. Enable? (y/n): ").lower() in ['y','yes'] and sp.run(['sudo','systemsetup','-setremotelogin','on']) or _die("x enable Remote Login")); u,p = os.environ.get('USER','user'), _sshd_port(); ip = sp.run("hostname -I 2>/dev/null | awk '{print $1}'", shell=True, capture_output=True, text=True).stdout.strip() or _sshd_ip(); h = f"{u}@{ip}" + (f":{p}" if p != 22 else ""); n = (sys.argv[3:] or [u])[0]; pw=input("Pw: ").strip(); sp.run(['sshpass','-p',pw,'ssh','-o','StrictHostKeyChecking=no','localhost','exit'],capture_output=True).returncode and _die("x pw"); _pw(n,pw); (c:=db()).execute("INSERT OR REPLACE INTO ssh(name,host)VALUES(?,?)",(n,h)); c.commit(); emit_event("ssh","add",{"name":n,"host":h,"pw":pw}); db_sync(); print(f"✓ {n}={h}"); return
    ...
    if wda in ('all','*') and len(sys.argv)>3:
        ...
        def _run(nh): n,h=nh; pw=_pw(n); hp=h.rsplit(':',1); r=sp.run((['sshpass','-p',pw] if pw else [])+['ssh','-o','ConnectTimeout=5','-o','StrictHostKeyChecking=accept-new']+(['-p',hp[1]] if len(hp)>1 else [])+[hp[0],cmd],capture_output=True,text=True); return(n,r.returncode==0,(r.stdout or r.stderr).split('\n')[0][:50])
