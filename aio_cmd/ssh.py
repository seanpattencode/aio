...
def run():
    ...
    if wda == 'start': r = sp.run('sshd' if os.getenv('TERMUX_VERSION') else 'sudo mkdir -p /run/sshd; sudo /usr/sbin/sshd', shell=True, capture_output=True, text=True); print(f"✓ sshd (port {_sshd_port()})") if r.returncode == 0 or _sshd_running() else print(f"x sshd: {r.stderr.strip() or 'install openssh-server'}"); return
    ...
    if wda == 'self': u,p = os.getenv('USER','user'), _sshd_port(); ip = sp.run("hostname -I 2>/dev/null | awk '{print $1}'", shell=True, capture_output=True, text=True).stdout.strip() or _sshd_ip(); h = f"{u}@{ip}" + (f":{p}" if p != 22 else ""); n = sys.argv[3] if len(sys.argv) > 3 else os.getenv('HOSTNAME', 'self'); (c:=db()).execute("INSERT OR REPLACE INTO ssh(name,host) VALUES(?,?)",(n,h)); c.commit(); emit_event("ssh","add",{"name":n,"host":h}); db_sync(); print(f"✓ {n}={h}"); return
    if wda in ('info','i'): [print(f"{n}: ssh {'-p '+hp[1]+' ' if len(hp:=h.rsplit(':',1))>1 else ''}{hp[0]}") for n,h in hosts]; return
    if wda == 'rm' and len(sys.argv) > 3: n=sys.argv[3]; _pw(n) and kr and kr.delete_password('aio-ssh',n); (c:=db()).execute("DELETE FROM ssh WHERE name=?",(n,)); c.commit(); emit_event("ssh","archive",{"name":n}); db_sync(); print(f"✓ rm {n}"); return
