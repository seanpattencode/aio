...
def _sync_repo(path, repo_name, msg='sync'):
    ...
    if (path/'.git').exists():
        sp.run(f'{g}git checkout -q main 2>/dev/null;git add -A&&git commit -qm "{msg}"',shell=True,capture_output=True)
        r=sp.run(f'{g}git pull -q --rebase origin main&&git push -q',shell=True,capture_output=True,text=True)
    else:
