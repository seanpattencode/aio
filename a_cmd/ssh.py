...
def run():
    ...
    if wda=='push-auth': [sp.run([sys.executable,__file__.replace('a_cmd/ssh.py','a.py'),'ssh',sys.argv[3],f"mkdir -p ~/.config/{d}&&echo {base64.b64encode((Path.home()/'.config'/d/f).read_bytes()).decode()}|base64 -d>~/.config/{d}/{f}"+";touch ~/.local/share/a/.auth_shared"*(d<'q')]) for d,f in[('rclone','rclone.conf'),('gh','hosts.yml')]]; print("✓"); return
    if wda == 'self':
